...
plot_labels = c("PD2019_ex23andMe" = "European PD GWAS",
                ...
                "Quach_2016_monocyte_naive" = "Monocyte",
                "Mono-cd14_NS" = "Monocyte CD14+",
                "expression_celltype_Monocyte_NS" = "Monocyte",
                "expression_subtype_Mono-cd14_NS" = "Monocyte CD14+",
...
bin1_jansen_summary_plot <- cowplot::plot_grid(NDD_gene_heatmap, bin1_jansen_aquino_locus_plot, 
                   ncol = 1,
                   labels = "auto")

ggsave(bin1_jansen_summary_plot,
       file = file.path(base_dir, "figures/thesis_jansen_aquino_bin1_summary_plot.png"),
...
rizig_aquino_barchart <- results %>% 
    dplyr::filter(significant != "Not_signif",
                Eqtl_type == "expression") %>%
    dplyr::mutate(eqtl_name = "Aquino 2023") %>% 
  dplyr::select(GWAS_1, eqtl_name, gene_name, dataset, PPH3 = PP.H3.abf, PPH4 = PP.H4.abf) %>%
  pivot_longer(c(PPH3, PPH4), names_to = "Hypothesis", values_to = "Posterior_value" ) %>%
  ggplot(aes(x = dataset, y = Posterior_value, fill = Hypothesis)) +
    geom_bar(stat = "identity", position  = "dodge") +
...
quach_barchart <- results %>% 
     separate(dataset, 
           into = c("Eqtl_type", "Cell_resolution", "Cell_name", "Treatment"), 
           sep = "_",
           remove = F) %>% 
    dplyr::filter(Cell_name == "monocyte",
                  Treatment == "naive") %>% 
    dplyr::mutate(eqtl_name = "Quach 2016") %>% 
    dplyr::select(GWAS_1, eqtl_name, gene_name, dataset, PPH3 = PP.H3.abf, PPH4 = PP.H4.abf) %>%
  pivot_longer(c(PPH3, PPH4), names_to = "Hypothesis", values_to = "Posterior_value" ) %>%
  ggplot(aes(x = dataset, y = Posterior_value, fill = Hypothesis)) +
    geom_bar(stat = "identity", position  = "dodge") +
...
african_pd_barchart <- (rizig_aquino_barchart + labs(x = NULL) | 
        quach_barchart + labs(x = NULL, y = NULL)) +
    plot_layout(guides = "collect", widths = c(2,1)) +
    plot_annotation(tag_levels = 'a')

ggsave(african_pd_barchart,
       height = 12, width = 20, units = "cm",
       file = file.path(base_dir, "figures/african_pd_barchart.png"))

ggsave(african_pd_barchart,
       height = 12, width = 20, units = "cm",
       file = file.path(base_dir, "figures/african_pd_barchart.pdf"))
