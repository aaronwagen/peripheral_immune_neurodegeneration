...
individuals_plot <- ggplot(gwas_studies, aes(x = "N_cases", y = fct_rev(Study))) +
  ...
  scale_size_continuous(range = c(1, 8), name = "N cases") +
  theme_minimal() +
  theme(axis.ticks.y = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        legend.key.size =  unit(0.4, 'lines')) +
...
aquino_gene_count <- aquino_gene_count_data %>% 
  ggplot(aes(x = disease, y = n, fill = fct_rev(genes_in_aquino))) +
...
## Enrichment of NDD genes in peripheral immune cells verse other genes

To compare whether NDD genes are enriched in peripheral immune cells, compared to other genes. Wi relative to the median expression of genes expressed in GTEX 
...
To identify the specificity of gene expression by cell type and activation state, a specificity metric tau is used. The metric rangers from 0-1, with a higher value indicating greater specificity.
...
euro_vs_african_expression_plot +
  labs(title = "DGE of cell lineages by ethnic background", 
