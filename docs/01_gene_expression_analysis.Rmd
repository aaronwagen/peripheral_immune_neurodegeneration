...
plot_labels = c("PD2019_ex23andMe" = "European PD GWAS",
                ...
                "brain_gtex_genes" = "Brain genes",
                 'all_gtex_genes_in_aqiuno' = "All genes",
                 "brain_gtex_genes_in_aquino" = "Brain genes")
...
my_palette <- c("Colocalised" = "darkslategrey",
             ...
             "Blood count association" = "darkslateblue",
             "No association" = "#7399BD",
             ...
             'all_gtex_genes_in_aqiuno' = "indianred4",
             "brain_gtex_genes" = "indianred3",
             "brain_gtex_genes_in_aquino" = "indianred3")
...
chen_gene_count_horizontal <- chen_gene_count_data %>% 
    dplyr::select(disease, Trait, `Blood count association`, `No association`) %>% 
    pivot_longer(cols = c("Blood count association", "No association"), names_to = "association", values_to = "count") %>% 
    dplyr::mutate(disease = factor(disease, levels = c("AD", "PD", "FTD"))) %>% 
    ggplot(aes(x = count, y = fct_rev(disease), fill = fct_rev(association))) +
    geom_bar(stat = "identity", position = "fill") +
    scale_fill_manual(values = my_palette,
                       breaks = c("Blood count association", "No association")) +
    scale_x_continuous(
        breaks = c(0, 0.25, 0.5, 0.75, 1),  # Set specific break points
        labels = c("0", "0.25", "0.5", "0.75", "1")  # Customize the labels
    ) +
    labs(y = NULL, x = "Proportion of genes", fill = NULL) +
    theme_aw +
    theme(legend.position = "bottom",
          axis.text.x = element_text(angle = 0, hjust = 0.5)) +
    facet_grid(~ Trait)
...
full_aquino_expression_plot <- filtered_data %>%  ggplot(aes(x = virus, y = fct_rev(Symbol), fill = proportion_expression_per_gene)) +
  geom_tile() +
  facet_nested(disease ~ Lineage + Cell,
             scales = "free",
             space = "free",
    ...
    scale_fill_viridis_c(
        ...
        labels = c("0", "0.25", "0.5", "0.75", "1"),  # Customize the labels
         guide = guide_colorbar(
             title.position = "top",     # Position title above the color bar
             title.hjust = 0,          # Center the title horizontally
             direction = "horizontal"   # Keep the color bar horizontal
        #     label.position = "bottom"   # Position labels below the color bar
             )) +
    labs(x = "Activation state",
         fill = "Proportion expression",
         y = "Gene")

ggsave(full_aquino_expression_plot,
       file = file.path(base_dir, "figures/full_aquino_expression_plot.png"),
       width = 39, height = 28, units = "cm")

full_aquino_expression_plot +
...
cell_tau_plot <- cell_tau %>%
    ...
    scale_fill_gradient(low = "grey90", high = "darkslategrey", limits = c(0, 1),
                        guide = guide_colorbar(
                            title.position = "top",     # Position title above the color bar
...
```{r}
# Set up data required for bootstrapping
...
NDD_geneID_in_aquino <- gtex_gene_expression %>% 
    dplyr::filter(symbol %in% genes_in_aquino$gene) %>% 
    dplyr::pull(gene) %>% sort()

saveRDS(NDD_geneID_in_aquino,
        file = file.path(base_dir, "/derived_data/NDD_ENSG_gene_ids_in_aquino.RDS"))
```
...
saveRDS(results_df,
          file =  file.path(base_dir, "/derived_data/bootstrap_expression.RDS"))
...
results_df <- readRDS(file = file.path(base_dir, "/derived_data/bootstrap_expression.RDS"))
gt(results_df)
...
# Data to help plotting median lines
...
expression_enrichment_plot <- (aquino_gene_count +
...
expression_enrichment_plot

ggsave(enrichment_plot,
       file = file.path(base_dir, "figures/thesis_gene_expression_enrichment_2.pdf"),
       height = 14, width = 32, units = "cm")

ggsave(enrichment_plot,
       file = file.path(base_dir, "figures/thesis_gene_expression_enrichment_2.png"),
       height = 14, width = 32, units = "cm")
...
This was undertaken using the opentargets API with a separate script. 500 bootstraps were chosen due to the limitations of the API, run with the following script
```{r, eval = F}

source(file.path(base_dir, "r_scripts/opentargets_chen_bootstrap_parallelised.R"))
```
...
geneIds <- NDD_geneID_in_aquino
  ...
  # Loop over each gene ID
    ...
    true_chen_associations <- filtered_df %>%
...
saveRDS(true_chen_associations,
        file = file.path(base_dir, "derived_data/chen_aquino_associations_true_result"))
...
true_chen_associations <- readRDS(file = file.path(base_dir, "derived_data/chen_aquino_associations_true_result"))

# Load preprocessed bootstrap data
allgene_bootstrap <- read_csv(file = file.path(base_dir,  "derived_data/chen_aquino_bootstrap_all_gtex_genes_parallel.csv"))

brain_bootstrap <- read_csv(file = file.path(base_dir,  "derived_data/chen_aquino_bootstrap_brain_gtex_genes_parallel.csv"))

chen_enrichment_results <- list(all_gtex_genes = allgene_bootstrap,
                               brain_gtex_genes = brain_bootstrap)

n_iterations = 500
n_genes_tested = 32

chen_cell_types <- c("White blood cell count",
                "Lymphocyte counts",
                "Monocyte count",
                "Neutrophil count")
...
chen_results_summary <- tibble()

for (bg_name in names(chen_enrichment_results)) {
   # bg_name <- names(chen_enrichment_results)[1]
    
    print(str_c("Background is: ", bg_name))
    
    for (chen_cell_type in chen_cell_types) {
     #   chen_cell_type <- chen_cell_types[1]
        

        print(str_c("Chen cell type is: ", chen_cell_type))
 
        true_result <- true_chen_associations %>% 
            pull(chen_cell_type)
                
        random_counts <- chen_enrichment_results[[bg_name]] %>% 
            pull(chen_cell_type)
        
        # Median number of expressed genes in random samples
        median_random_associations <- median(random_counts)
        
        # Standard deviation
        sd_random_associations <- sd(random_counts)
        ...
        # Empirical p-value
        p_value_empirical <- sum(random_counts >= true_result) / n_iterations
        
        results_df <- tibble(
        background = bg_name,
        chen_cell = chen_cell_type,
        n_genes_tested = n_genes_tested,
        true_result_associations = true_result,
        median_random_expressed = median_random_expressed,
        sd_random_expressed = sd_random_expressed,
        ci_lower = as.numeric(ci_lower),
        ci_upper = as.numeric(ci_upper),
        ...
        if (nrow(chen_results_summary) == 0) {
            chen_results_summary <- results_df
        } else {
            chen_results_summary <- bind_rows(chen_results_summary,
                                              results_df)
        }
...
chen_results_summary

saveRDS(chen_results_summary,
          file =  file.path(base_dir, "/derived_data/bootstrap_chen_associations.RDS"))
...
chen_results_summary <- readRDS(file =  file.path(base_dir, "/derived_data/bootstrap_chen_associations.RDS"))

# Prepare data for density plots
enrichment_histogram_data <- chen_results_summary %>% 
  unnest(cols = c(raw_values)) %>% 
  dplyr::select(background, chen_cell, raw_values, true_result_associations, p_value_empirical)

# Calculate median and max density for each chen_cell and background
median_data <- enrichment_histogram_data %>%
  group_by(background, chen_cell) %>%
    summarize(
        median_value = median(raw_values),
        max_density = {
            d <- density(raw_values)
            approx(d$x, d$y, xout = median(raw_values))$y
        }
  )

# Merge p-values and true_result_associations for annotations
annotation_data <- enrichment_histogram_data %>%
  group_by(background, chen_cell) %>%
  summarize(
    true_result = unique(true_result_associations),
    p_value_text = ifelse(
      unique(p_value_empirical) == 0.0, 
      "p-value < 0.002", 
      paste0("p-value = ", formatC(unique(p_value_empirical), format = "f", digits = 3))
    )
  )

# Filter annotations for specific backgrounds
all_genes_annotations <- annotation_data %>% filter(background == "all_gtex_genes")
brain_genes_annotations <- annotation_data %>% filter(background == "brain_gtex_genes")

# Plot with facet_wrap
chen_associations_bootstrap <- enrichment_histogram_data %>% 
  ggplot(aes(x = raw_values, colour = background)) + 
  geom_density(linewidth = 1, show.legend = FALSE) +
  
  # Add true result line for each facet
  geom_vline(data = annotation_data, aes(xintercept = true_result), 
             colour = "darkslategrey", linewidth = 1, linetype = "dashed") +
  
  # Add true result text annotations
  geom_text(data = annotation_data, aes(x = true_result, y = Inf, label = "\nAssociation count"), 
             size = 4, hjust = 1.05, angle = 90, colour = "darkslateblue") +
  
  # Add p-value annotations for all_gtex_genes in indianred4
  geom_text(data = all_genes_annotations, aes(x = 1, y = 0.109, label = p_value_text), 
            inherit.aes = FALSE, hjust = 0, vjust = 0, colour = "indianred4", size = 4) +
  
  # Add p-value annotations for brain_gtex_genes in indianred3
  geom_text(data = brain_genes_annotations, aes(x = 1, y = 0.102, label = p_value_text), 
            inherit.aes = FALSE, hjust = 0, vjust = 0, colour = "indianred3", size = 4) +
  
  # Scale, labels, and themes
  scale_x_continuous(limits = c(0, 50), breaks = c(0, 10, 20, 30, 40, 50)) +
    scale_y_continuous(limits = c(0, 0.11)) +
  scale_colour_manual(values = my_palette, labels = plot_labeller) +
  labs(x = "Number of associations", y = "Density", colour = "Background") +
  
  # Add median line for each background
  geom_segment(data = median_data, aes(x = median_value, xend = median_value, 
                                       colour = background, y = 0, yend = max_density), 
               linetype = "dashed", linewidth = 1) +
  
  # Add facets
  facet_wrap(~ chen_cell, nrow = 1) +
...
chen_associations_bootstrap

```



## Enrichment of maximal gene expression in innate immune cells
...
# Create filtered dataframe of those cells with max expression in innate immune cells (over all activation states)
long_data_innate_filtered <- long_data %>% 
    group_by(GeneID, Symbol) %>% 
    slice_max(TPM, n = 1, with_ties = TRUE) %>% 
    ungroup() %>% 
    dplyr::mutate(max_innate = case_when(Cell %in% innate_cells ~ 1,
                                         TRUE ~ 0)) %>% 
    group_by(GeneID, Symbol) %>% 
    slice_max(max_innate, n=1, with_ties = F) %>%  # Remove double entries per gene (if happen to have equal TPM) selecting any with max in innate immune cells
    dplyr::select(GeneID, Symbol, max_innate)
    
long_data_innate_filtered


all_gtex_genes_in_aqiuno <- intersect(all_gtex_genes, long_data_innate_filtered$GeneID)
brain_gtex_genes_in_aquino <- intersect(brain_gtex_genes, long_data_innate_filtered$GeneID)
innate_enrichment_backgrounds <- list(all_gtex_genes_in_aqiuno = all_gtex_genes_in_aqiuno,
                               brain_gtex_genes_in_aquino = brain_gtex_genes_in_aquino)
target_genes <- NDD_geneID_in_aquino
n_genes_tested = length(target_genes)


true_result_innate <- long_data_innate_filtered %>% 
    dplyr::filter(GeneID %in% target_genes) %>% 
    pull(max_innate) %>% 
    sum()
...
n_iterations = 1000
 ...
 for (bg_name in names(innate_enrichment_backgrounds)) {
   # bg_name <- names(innate_enrichment_backgrounds)[1]
    gene_background <- innate_enrichment_backgrounds[[bg_name]]
    ...
    random_counts <- replicate(n_iterations, {
      ...
      # Count the number of sampled genes that have maximal expression in innate immune cells
      number_of_max_innate_genes <- long_data_innate_filtered %>% 
          dplyr::filter(GeneID %in% random_sample) %>% 
          pull(max_innate) %>% 
          sum()  })
    ...
    # Empirical p-value
    p_value_empirical <- sum(random_counts >= true_result_innate) / n_iterations
    ...
    result_df <- tibble(
        background = bg_name,
        n_genes_tested = n_genes_tested,
        true_result_innate_max = true_result_innate,
        median_random_expressed = median_random_expressed,
        sd_random_expressed = sd_random_expressed,
        ci_lower = as.numeric(ci_lower),
        ci_upper = as.numeric(ci_upper),
...
saveRDS(results_df,
          file =  file.path(base_dir, "/derived_data/bootstrap_innate_max_expression.RDS"))
...
results_df <- readRDS(file =  file.path(base_dir, "/derived_data/bootstrap_innate_max_expression.RDS"))
...
innate_enrichment_expression <- enrichment_histogram_data %>% 
    ggplot(aes(x = raw_values, colour = background)) + 
    geom_density(linewidth = 1, show.legend = FALSE) +
    geom_vline(xintercept = true_result_innate, colour = "darkslategrey", linewidth = 1, linetype = "dashed") +
    annotate("text", x = true_result_innate, y = Inf, size = 5, label = "\nInnate cell max", hjust = 1.1, angle= 90, colour = "darkslategrey") +
        annotate("text", x = 0,  y = 0.99 * max(median_data$max_density),
             hjust = 0,
             label = "p-value = 0.009",
             colour = "indianred4", size = 4) +
    annotate("text", x = 0,  y = 0.92 * max(median_data$max_density),
             label = "p-value = 0.007",
              hjust = 0,
             colour = "indianred3", size = 4) +
    scale_x_continuous(limits = c(0,32),
                       breaks = c(0,10, 20, 30, 32)) +
    scale_colour_manual(values = my_palette, labels = plot_labeller) +
    labs(x = "Number of genes", y = "Density", colour = "Background") +
    geom_segment(data = median_data, aes(x = median_value, 
                                         xend =  median_value, 
...
innate_enrichment_expression

ggsave(innate_enrichment_expression,
       file = file.path(base_dir, "/figures/innate_bootstrap_results.png"),
       units = "cm", height = 13, width = 18)
       

ggsave(innate_enrichment_expression,
       file = file.path(base_dir, "/figures/innate_bootstrap_results.pdf"),
       units = "cm", height = 13, width = 18)
...
euro_african_full_plot <- table_4b %>% 
    dplyr::mutate(Condition = factor(Condition, levels = c("NS", "IAV", "COV"))) %>% 
     ggplot(aes(x =Condition, 
               y = fct_rev(Symbol), 
               fill = beta_lineage_adj)) +
    geom_tile() +
    facet_grid(disease ~ fct_relevel(Lineage, aquino_cell_order$Lineage),
               scales = "free",
               space = "free") +
...
ggsave(euro_african_full_plot,
        file =  file.path(base_dir, "figures/euro_african_full_expression_plot.pdf"),
       height = 20, width = 15, units = "cm")
       
ggsave(euro_african_full_plot,
        file =  file.path(base_dir, "figures/euro_african_full_expression_plot.png"),
       height = 20, width = 15, units = "cm")
...
fig1_gene_expression_bootstrap <- (
    ( aquino_gene_count | 
          enrichment_expression +
          labs(x = NULL) +
          theme(
              axis.text.x = element_text(angle = 0, hjust = 0.5))) /
        plot_spacer() /
        (plot_spacer() | total_aquino_gene_count + 
             theme(axis.text.x = element_text(angle = 0, hjust = 0.5))) /
        chen_gene_count_horizontal /
        chen_associations_bootstrap +
        theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
) + plot_layout(heights = c(6,-1.3, 0.7,  3, 6),
                guides = "collect") +
...
ggsave(fig1_gene_expression_bootstrap,
       file =  file.path(base_dir, "figures/fig_1_gene_expression_bootstrap.pdf"),
       height = 30, width = 29, units = "cm")

ggsave(fig1_gene_expression_bootstrap,
       file =  file.path(base_dir, "figures/fig_1_gene_expression_bootstrap.png"),
       height = 30, width = 29, units = "cm")
```



```{r, fig.width = 18, fig.height= 14, units = "cm"}


fig2_aquino_gene_expression <-  ((aquino_cell_expression_plot_max  + theme_aw + 
      ...
      ( cell_tau_plot + theme_aw +theme(strip.text.y = element_blank(),
       ...
       activationState_tau_plot + labs(x = NULL) + theme_aw +  theme(axis.text.y = element_blank(),
          ...
          plot_layout(guides = "collect", widths = c(5,-1.1, 5))  |
   (euro_vs_african_expression_plot / plot_spacer()))) +
      plot_layout(widths = c(12,4.3, 3)) +
    plot_annotation(tag_levels = "a")

fig2_aquino_gene_expression 

ggsave(fig2_aquino_gene_expression,
       height = 30, width = 39, units = "cm",
       file = file.path(base_dir, "figures/fig2_aquino_gene_expression.pdf"))

ggsave(fig2_aquino_gene_expression,
       height = 30, width = 39, units = "cm",
       file = file.path(base_dir, "figures/fig2_aquino_gene_expression.png"))
```
