...
if (location == "jane") {
  ...
  base_dir <- "/nemo/lab/gandhis/home/users/wagena/SNCA"
}
...
# Assign arguments
args <- list(
  gwas_name = "shigemizu_2021",
  munged_gwas = file.path(base_dir, "raw_data/GWAS/coloc_munged_data/shigemizu2021_gwas_tidy_maf.txt"),
  ...
  aquino_summary_table = file.path(base_dir, "raw_data/GWAS/aquino_2023_covid_eqtl/aquino_summary_table.csv"),
  output_path = file.path(base_dir, 
                          "processed_data",
                           "shigemizu_aquino",
                           "coloc"),
  # PD_gwas_details = list(n_cases = 33674, 
  #                        n_controls = 449056, 
  #                        cc_or_quant = "cc", 
  #                        n_total = 482730, 
  #                        prop_cases = 0.074989),
  ensembl_gene_ids_overlapping_1Mb_window_hit_path =  file.path(base_dir, 
                                                                "raw_data",
                                                                 "GWAS/coloc_munged_data/shigemizu_ensembl_gene_ids_overlapping_1Mb_window_hit.RDS"),
  gtf_38.84 = file.path(base_dir, "raw_data/GRCh38/annotation/Homo_sapiens.GRCh38.84.gtf.gz")
)
...
# Jansen GWAS: 
# This has been tidied in the specific script: "r_script/01_munge_GWASs_for_coloc.R"
# Note that this GWAS is in GRCh37. We will use the rs number to define SNPs
...
## Load eQTL list previously derived

eqtl_df <- fread(args$aquino_summary_table)


### The code to derive this table is as below:
...
eqtl <- eqtl %>% # Perform minimal munging on overall qtl here to allow for munging at a smaller scale within loops below. Note: raw p values are required, not FDR corrected
  dplyr::select(SNP = rsID, CHROM, POS_B38, gene, Al2 = REF, Al1 = ALT, celltype, condition, beta = slope, se = slope_se, p.value = pvalue) 
  ...
  for(k in 1:length(p12)){
    ...
    coloc_results_annotated <-
      colochelpR::get_coloc_results(df1 = gwas_tidy, df2 = eqtl_tidy_gene_filtered, 
                                    # Harmonise set to false as using p value for the gwas, so can't flip the beta
                                    harmonise = F, 
                                    df1_type = "cc", df2_type = "quant", 
                                    df1_beta_or_pval = "pval", df2_beta_or_pval = "beta",
                                    df_1_propor_cases = max(gwas_tidy$prop_cases),
                                    df1_N = max(gwas_tidy$N),
