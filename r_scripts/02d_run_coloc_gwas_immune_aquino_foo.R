...
#here::i_am("r_scripts/02f_run_coloc_gwas_immune_aquino_Nalls.R")
...
args <- list(
  gwas_name = "foo2020",
  munged_gwas = "/nemo/lab/gandhis/home/users/wagena/references/GWAS/coloc_munged_data/foo2020_gwas_tidy_varbeta.txt",
  PD_gwas_details = list(n_cases = 6724,
                      n_controls = 24581,
                      n_total = 31305,
                      prop_cases = 0.214790,
                      cc_or_quant = "cc"),
  ...
  output_path = here::here("processed_data",
                           "foo_aquino",
                           "coloc"),
  ensembl_gene_ids_overlapping_1Mb_window_hit_path =  here::here("raw_data/GWAS/coloc_munged_data/foo_ensembl_gene_ids_overlapping_1Mb_window_hit.RDS"),
...
# For the aquino eqtl will use rs numbers as the SNP identifier
gwas_tidy <- gwas_tidy %>% 
  dplyr::select(-SNP) %>% 
  dplyr::rename(SNP = rs_number)
...
# Note in this approach the p value is corrected - this is actually NOT the correct approach for coloc analyses. However, given I have used beta as entries, the p value is not taken into account.
